...
final class LoadAdmin extends AdminPages {
	...
	private function construct() {
		...
		\add_action( 'admin_notices', [ $this, '_do_migration_notice' ] );
	...
	/**
	 * Adds dashboard notice for when the user is running an outdated version of the plugin.
	 *
	 * @since 1.5.3
	 */
	public function _do_migration_notice() {

		if ( ! \current_user_can( 'install_plugins' ) || ! $this->is_plugin_activated() )
			return;

		$current_screen = \get_current_screen();

		$show = ( ! empty( $current_screen->base ) && in_array( $current_screen->base, [ 'dashboard', 'update-core', 'plugins' ], true ) )
			 || ( ! empty( $current_screen->parent_base ) && \the_seo_framework()->seo_settings_page_slug === $current_screen->parent_base );
		if ( ! $show ) return;
		...
		if ( $time < strtotime( '2018-11-14' ) ) return;

		if ( $time < strtotime( '2018-11-21' ) ) {
			$text = $this->convert_markdown(
				sprintf(
					'"The SEO Framework - Extension Manager" plugin is going [to be removed from WordPress.org](%s). Learn about our [new plugin here](%s).',
					'https://theseoframework.com/?p=2747',
					'https://theseoframework.com/?p=2760'
				),
				[ 'a' ]
			);
		} elseif ( $time < strtotime( '2018-12-01' ) ) {
			$text = $this->convert_markdown(
				sprintf(
					'"The SEO Framework - Extension Manager" plugin is going [to be removed from WordPress.org](%s) and the API service for this plugin will stop in 2019. Learn about our [new plugin here](%s).',
					'https://theseoframework.com/?p=2747',
					'https://theseoframework.com/?p=2760'
				),
				[ 'a' ]
			);
		} elseif ( $time < strtotime( '2019-01-01' ) ) {
			$text = $this->convert_markdown(
				sprintf(
					'"The SEO Framework - Extension Manager" plugin is going [to be removed from WordPress.org](%s) and the API service for this plugin will stop in 2019. Learn about our [new plugin here](%s), and benefit from [our discount until December 31st, 2018](%s).',
					'https://theseoframework.com/?p=2747',
					'https://theseoframework.com/?p=2760',
					'https://theseoframework.com/?p=2748'
				),
				[ 'a' ]
			);
		} else {
			$text = $this->convert_markdown(
				sprintf(
					'"The SEO Framework - Extension Manager" plugin can no longer be updated and the API no longer works. This is a security risk. Get the [new plugin here](%s).',
					'https://theseoframework.com/?p=2760'
				),
				[ 'a' ]
			);
		...
		$text and \the_seo_framework()->do_dismissible_notice( $text, $type, true, false );
	}
